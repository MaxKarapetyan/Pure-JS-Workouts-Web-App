...
class App {
  ...
  _loadMap(position) {
    ...
    // leaflet map API config

    this.#map = L.map('map').setView(coords, this.#mapZoovLevel);
  ...
  _newWorkout(e) {
    ...
    // Hide list for and Clear input fields
    this.hideForm();
  ...
  _moveToPopup(e) {
    ...
    this.#map.setView(workout.coords, this.#mapZoovLevel, {
      animate: true,
      pan: { duration: 1 },
    });
  }
